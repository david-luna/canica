...
import {
  GradeRecordDataTransfer,
  SchoolClassRecordDataTransfer,
} from "../data-transfer";
...
export class WebScrappingService {
  ...
  async execute(options = { debug: false }): Promise<void> {
    ...
    try {
      ...
      const gradesData = await this.getGrades();

      for (let i = 0; i < classData.length; i++) {
        const currentClass = classData[i];

        await this.getClassStudents(page, currentClass);
        currentClass.students.forEach(
          (student) => (student.grades = gradesData)
        );
  ...
  private async listClasses(
    page: puppeteer.Page
  ): Promise<SchoolClassRecordDataTransfer[]> {
  ...
  private async getGrades(): Promise<GradeRecordDataTransfer[]> {
